...
rubric item — with up to 500 images in one call, re-checked in a
...
def _rubric_input_char_budget() -> int:
    ...
    budget, inputs are dropped (judge sees none). Override with
    GDPVAL_RUBRIC_INPUT_CHARS."""
...
def _find_deliverable_view(task: Task, episode: Episode) -> DeliverableView:
    ...
    for c in _rank_candidates(_deliverable_candidate_paths(episode), task):
        ...
        if rendered:
            text += (
                f"\n\n[Attached visual content: {Path(c).name} is rendered as "
                f"{len(rendered)} consecutive page image(s) below.]"
            )
        texts.append(text)
        names.append(Path(c).name)
...
def _find_input_view(task: Task) -> DeliverableView:
    ...
    in memory. Inputs are omitted when their text exceeds the input budget.
    """
    ...
    for path in _input_file_paths(task):
        ...
        if rendered:
            t += (
                f"\n\n[Attached visual content: {Path(path).name} is rendered as "
                f"{len(rendered)} consecutive page image(s) below.]"
            )
        sections.append(t)
        per_file.append(rendered)
    ...
    if not sections:
        return DeliverableView("", [], "none")
    if total > budget:
        logger.warning("[gdpval] input attachments %d chars > budget %d; judge will not see inputs", total, budget)
        return DeliverableView("", [], "none")
    images = _flatten_images(per_file, _max_judge_images())
...
def _grading_messages(
    ...
    if input_images:
        input_notes = (
            f"\n\n- {len(input_images)} input page image(s) are attached immediately below, "
            "in file order with consecutive pages."
        )
    input_section = f"## 2. Input Files\n{input_text or 'No input files were provided.'}{input_notes}"
    attachment_notes: list[str] = []
    if images:
        attachment_notes.append(
            f"{len(images)} page image(s) are attached immediately below, in file order with consecutive pages."
        )
