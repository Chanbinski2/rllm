...
from whatever the solver claimed. ffprobe runs on the host when present, else
inside the published GDPval image, which carries ffmpeg as part of AA's package
closure.
"""
...
# --------------------------------------------------------------------------- #
# ffprobe technical metadata (host binary -> docker fallback)
# --------------------------------------------------------------------------- #
...
def _ffprobe_json(path: Path) -> dict | None:
    ...
    Tries the host binary first, then the published image — falling back on
    *any* host failure, missing or present-but-broken, mirroring how the
    office->PDF converter degrades.
    """
    import shutil
    import subprocess
    ...
    if shutil.which("ffprobe"):
        attempts.append(("host", ["ffprobe", *args, str(path)]))
    src = path.resolve()
    attempts.append(
        (
            "docker",
            ["docker", "run", "--rm", "-v", f"{src.parent}:/input:ro", render_docker_image(), "ffprobe", *args, f"/input/{src.name}"],
        )
    )
    for label, cmd in attempts:
        try:
            out = subprocess.run(cmd, capture_output=True, timeout=120, check=True)
            return json.loads(out.stdout.decode("utf-8", errors="replace"))
        except Exception as e:  # noqa: BLE001 — try the next source; metadata is best-effort
            logger.warning("[gdpval] ffprobe (%s) failed for %s: %s", label, path.name, e)
    return None
