...
def test_view_image_can_be_withheld_from_a_text_only_model(monkeypatch):
    ...
    try:
        ...
        env = reloaded.StirrupHarness().build_env(_task(), _config())
...
def test_invocation_runs_stirrup_in_the_existing_sandbox():
    ...
    assert command.startswith("/opt/stirrup-venv/bin/python /opt/gdpval-aa/driver.py")
...
class _RecordingSandbox:
    ...
    def exec(self, command, timeout=None, user=None):
        ...
        if command.startswith("cat ") and RUN_METADATA_PATH in command:
...
def test_solver_runs_as_the_task_agent_user_and_config_is_written_as_root(tmp_path, monkeypatch):
    ...
    StirrupHarness().run(_task(tmp_path), _config(), env=sandbox)

    invocation_users = [user for command, user in sandbox.execs if "/opt/stirrup-venv/bin/python /opt/gdpval-aa/driver.py" in command]
    assert invocation_users == ["user"]

    # /opt is not writable by the solver, so config files are staged as root.
    config_writes = [user for command, user in sandbox.execs if "cat > /opt/gdpval-aa/" in command]
...
def test_system_prompt_is_written_into_the_sandbox_verbatim(tmp_path, monkeypatch):
    ...
    write = next(command for command, _ in sandbox.execs if "cat > /opt/gdpval-aa/system_prompt.txt" in command)
    assert aa.AA_GDPVAL_SYSTEM_PROMPT in write

    instruction_write = next(command for command, _ in sandbox.execs if "cat > /opt/gdpval-aa/instruction.txt" in command)
...
def test_manifest_never_reports_a_quality_score(tmp_path, monkeypatch):
    ...
    episode = StirrupHarness().run(_task(tmp_path), _config(), env=sandbox)
...
def test_a_run_that_produced_nothing_is_still_recorded(tmp_path, monkeypatch):
    ...
    episode = StirrupHarness().run(_task(tmp_path), _config(), env=_DeadSandbox())
